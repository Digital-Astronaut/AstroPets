...
public class MobHelmetChanceEditor extends PaginatedMenu {

    private AstroMob selectedMob;

    public MobHelmetChanceEditor(PlayerMenuUtility playerMenuUtility, AstroMob selectedMob, Menu previousMenu) {
    ...
    public void loopData(Object o) {
        ...
        if (o instanceof Material m) {
            ...
            if (selectedMob.getMobHelmetChance().getCurrentMemoryValue() == (index + 1)) {
    ...
    public int getMaxAmountOfDisplayedItems() {
        ...
        return selectedMob.getMobHelmetChance().getMaximumValue();
    }

    @Override
    public String getMenuName() {
        return ChatColor.BLUE + "" + ChatColor.BOLD + "Helmet Chance";
    }
        ...
        if (e.getRawSlot() < 45) {
            if (e.getCurrentItem().hasItemMeta()) {
                if (e.getCurrentItem().getItemMeta().hasDisplayName()) {
                    selectedMob.getMobHelmetChance().setCurrentMemoryValue(Integer.parseInt(ChatColor.stripColor(e.getCurrentItem().getItemMeta().getDisplayName())));
                }
                open();
            }
        }
