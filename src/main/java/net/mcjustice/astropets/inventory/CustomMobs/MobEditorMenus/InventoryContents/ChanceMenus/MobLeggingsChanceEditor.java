...
public class MobLeggingsChanceEditor extends PaginatedMenu {

    private AstroMob selectedMob;

    public MobLeggingsChanceEditor(PlayerMenuUtility playerMenuUtility, AstroMob selectedMob, Menu previousMenu) {
    ...
    public void loopData(Object o) {
        ...
        if (o instanceof Material m) {
            ...
            if (selectedMob.getMobLeggingsChance().getCurrentMemoryValue() == (index + 1)) {
    ...
    public int getMaxAmountOfDisplayedItems() {
        ...
        return selectedMob.getMobLeggingsChance().getMaximumValue();
    }

    @Override
    public String getMenuName() {
        return ChatColor.BLUE + "" + ChatColor.BOLD + "Leggings Chance";
    }
        ...
        if (e.getRawSlot() < 45) {
            if (e.getCurrentItem().hasItemMeta()) {
                if (e.getCurrentItem().getItemMeta().hasDisplayName()) {
                    selectedMob.getMobLeggingsChance().setCurrentMemoryValue(Integer.parseInt(ChatColor.stripColor(e.getCurrentItem().getItemMeta().getDisplayName())));
                }
                open();
            }
        }
