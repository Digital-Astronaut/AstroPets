...
public class MobMainHandChanceEditor extends PaginatedMenu {

    private AstroMob selectedMob;

    public MobMainHandChanceEditor(PlayerMenuUtility playerMenuUtility, AstroMob selectedMob, Menu previousMenu) {
    ...
    public void loopData(Object o) {
        ...
        if (o instanceof Material m) {
            ...
            if (selectedMob.getMobMainHandChance().getCurrentMemoryValue() == (index + 1)) {
    ...
    public int getMaxAmountOfDisplayedItems() {
        ...
        return selectedMob.getMobMainHandChance().getMaximumValue();
    }

    @Override
    public String getMenuName() {
        return ChatColor.BLUE + "" + ChatColor.BOLD + "Main Hand Item Chance";
    }
        ...
        if (e.getRawSlot() < 45) {
            if (e.getCurrentItem().hasItemMeta()) {
                if (e.getCurrentItem().getItemMeta().hasDisplayName()) {
                    selectedMob.getMobMainHandChance().setCurrentMemoryValue(Integer.parseInt(ChatColor.stripColor(e.getCurrentItem().getItemMeta().getDisplayName())));
                }
                open();
            }
        }
