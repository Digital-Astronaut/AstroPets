...
public class MobChestplateChanceEditor extends PaginatedMenu {

    private AstroMob selectedMob;

    public MobChestplateChanceEditor(PlayerMenuUtility playerMenuUtility, AstroMob selectedMob, Menu previousMenu) {
    ...
    public void loopData(Object o) {
        ...
        if (o instanceof Material m) {
            ...
            if (selectedMob.getMobChestplateChance().getCurrentMemoryValue() == (index + 1)) {
    ...
    public int getMaxAmountOfDisplayedItems() {
        ...
        return selectedMob.getMobChestplateChance().getMaximumValue();
    }

    @Override
    public String getMenuName() {
        return ChatColor.BLUE + "" + ChatColor.BOLD + "Chestplate Chance";
    }
        ...
        if (e.getRawSlot() < 45) {
            if (e.getCurrentItem().hasItemMeta()) {
                if (e.getCurrentItem().getItemMeta().hasDisplayName()) {
                    selectedMob.getMobChestplateChance().setCurrentMemoryValue(Integer.parseInt(ChatColor.stripColor(e.getCurrentItem().getItemMeta().getDisplayName())));
                }
                open();
            }
        }
